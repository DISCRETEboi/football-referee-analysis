...
cat("here: ", a, "\n"); a <- a + 1
rpr_nm <- function(name) {
  c <<- c + 1
  if (name == "") {as.character(c)} else {name}
}
b <- 1; c <- 0
read_data <- function(data_path) {
  cat("iteration: ", b, "\n"); b <<- b + 1
  data_ <- readr::read_csv(str_c("premier-league-data/", data_path), show_col_types = F, name_repair = "universal")
  vroom::problems(data_)
  return(data_)
}
cat("here: ", a, "\n"); a <- a + 1
